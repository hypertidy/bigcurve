...
#' @param ... ignored
#' @param source input coordinate system, default 'EPSG:4326'
#'   (axis-normalized, so input is lon,lat regardless of authority order)
#'
...
rproj_xy <- function(x, target, ..., source = NULL) {
  if (is.null(source)) source <- default_crs()
